...
const mostSummerWins = country => {
  return `SELECT year, COUNT(*) AS count FROM GoldMedal WHERE country = '${country}' and season = 'Summer'
  GROUP BY year
...
const mostWinterWins = country => {
  return `SELECT year, COUNT(*) AS count FROM GoldMedal WHERE country = '${country}' and season = 'Winter'
  GROUP BY year
  ORDER BY COUNT(*) DESC LIMIT 1;`;
};
...
const bestDiscipline = country => {
  return `SELECT year, discipline, COUNT(*) AS count FROM GoldMedal WHERE country = '${country}'
  GROUP BY discipline
  ORDER BY COUNT(*) DESC LIMIT 1`;
};
...
const bestSport = country => {
  return `SELECT year, sport, COUNT(*) AS count FROM GoldMedal WHERE country = '${country}'
  GROUP BY sport
  ORDER BY COUNT(*) DESC LIMIT 1`;
};
...
const bestEvent = country => {
  return `SELECT year, event, COUNT(*) AS count FROM GoldMedal WHERE country = '${country}'
  GROUP BY event
  ORDER BY COUNT(*) DESC LIMIT 1`;
};
